...
DELETE FROM PV_PC.PAGEVIEWS WHERE EVENT_ID IN
  (SELECT ROW_ID FROM
  (SELECT ROW_ID, ROW_NUMBER() OVER (PARTITION BY A.USER_ID,DATE_TRUNC('hour',EVENT_DATETIME) ORDER BY A.LOAD_DATE ASC) RNK FROM PV_PC.USERS_LOCATION  A ) WHERE RNK != 1);
